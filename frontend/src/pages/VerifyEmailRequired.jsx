...
import { FiMail, FiRefreshCw, FiLogOut, FiCheckCircle, FiArrowLeft, FiShield, FiSend } from 'react-icons/fi';
...
export default function VerifyEmailRequired() {
  ...
  const cooldownKey = useMemo(
    () => (user?.email ? `haitz_verify_resend_cooldown:${user.email.trim().toLowerCase()}` : 'haitz_verify_resend_cooldown'),
    [user?.email]
  );
  ...
  useEffect(() => {
    const savedUntil = Number(localStorage.getItem(cooldownKey) || 0);
    const tick = () => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (verified) {
        toast.success('Email verifie ! Bienvenue.');
      } else {
        toast.error("L'email n'est pas encore valide.");
      }
    } catch (err) {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Impossible de renvoyer le mail.');
    } finally {
      setSending(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-[#0a0a0a] font-sans">
      ...
      >
        <div className="backdrop-blur-2xl bg-white/5 border border-white/10 rounded-[2.5rem] shadow-2xl p-8 md:p-12 text-center relative overflow-hidden">
          ...
          <h1 className="text-3xl font-black text-white mb-4 tracking-tight">Verifiez votre email</h1>
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full py-4 bg-white/5 border border-white/10 text-white rounded-2xl font-bold transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white/10"
            >
              {sending ? <FiRefreshCw className="animate-spin" /> : <FiSend />}
              {cooldown > 0 ? `Renvoyer dans ${cooldown}s` : 'Renvoyer le mail de verification'}
            </button>
          ...
          <div className="mt-10 pt-8 border-t border-white/5">
            <div className="flex items-center justify-center gap-2 text-slate-600 text-[10px] font-bold uppercase tracking-[0.2em]">
              <FiShield /> Systeme securise par HAITZ
            </div>
          </div>
        </div>
      </motion.div>
